...
const { token, status, errorCode, executeChallenge, widgetRef, reset } =
  useTurnstile(props.siteKey);

defineExpose({ token, status, errorCode, executeChallenge, widgetRef, reset });
</script>
  ...
  <div class="relative">
    ...
    <div class="inline-flex items-center gap-2">
      ...
      <div v-else-if="status === 'expired' || status === 'error'" class="">
        ...
        <span
          v-if="status === 'error' && errorCode"
          class="ml-2 text-xs text-error/80"
        >
          code: {{ errorCode }}
        </span>
        <button
